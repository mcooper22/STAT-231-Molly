...
```{r}
justicevisual <- ggplot(data = justices2, mapping = aes(x = justice , y = tenure_length, fill = date_confirmed)) + geom_point() 

justicevisual + xlab("Justice Number") + ylab("Length of Tenure") + ggtitle("The Tenure Lengths of U.S. Supreme Court Judges")
```

> ANSWER: This visual demonstrates the tenure lengths of every US Supreme Court Justice. While the numbers are difficult to decipher given the size of the data set, it's arranged in order of confirmation. The scatterplot shows us that there is really no correlation between the date of confirmation and the length of one's tenure - it appears to be a fairly arbitrary connection and has not trended much throughout history.  
